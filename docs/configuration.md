...
|---|---|---|---|---|
| Gateway 选项 | Runtime 根目录、空闲回收、命名空间白名单 | 部署运维 | 重启宿主应用 | [Runtime Gateway 选项](#runtime-gateway-选项) |
...
| 字段 | 类型 | 默认值 | 说明 |
|---|---|---|---|
| `projectRoot` | string | 必填 | 宿主应用根目录，用于定位 Preset、Profile 与插件包 |
...
| `exposedSettingsNamespaces` | string[] | `DSHSERVER_EXPOSED_SETTINGS_NAMESPACES` 或 `[]` | 远程可见的设置命名空间 |

Gateway 直接读取的环境变量只有三个：`DSH_SOURCE_ROOT`、`DSH_RUNTIME_IDLE_MS`、
`DSH_RUNTIME_DISABLED`，外加 `DSHSERVER_EXPOSED_SETTINGS_NAMESPACES`（JSON 数组字符串，
格式非法时启动失败）。其余字段由调用方显式传入。
...
| `DSHSERVER_EXPOSED_SETTINGS_NAMESPACES` | Gateway 选项 | 远程可见的设置命名空间 JSON 数组 |
...
| 条目 | 设置 | 目的 |
|---|---|---|
| `agent-default-model` | 由环境变量注入 Provider 与模型 | 使用部署批准的默认模型 |
| `session-title-llm` | 禁用 | 避免额外的模型调用 |
| `llm-deepseek` / `llm-pi-ai` | 启用 | 保留官方 Provider |
| `settings` / `credentials` | `path` 指向租户目录 | 同租户共享配置与凭据 |
| `dshserver-integration-settings` | 插入 | 在 Host 平面安装租户设置命名空间 |
| `api-gateway` | `exposedSettingsNamespaces` | 限制远程可见的设置命名空间 |
