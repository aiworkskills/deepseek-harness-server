...
上表描述的是默认姿态：Agent 通过窄接口读写业务数据，本身不需要写任何文件，沙箱固定
Read Only。这是本仓库的原始设计，也是 `permissionMode` 的默认值。

但有一类 Agent 的产出**本身就是文件**——生成文档、代码、图片，或者需要在工作区里执行
脚本。这类场景下 Read Only 不是"更安全"，而是功能归零。`permissionMode:
'workspace-write'` 为它们保留了另一条路：Agent 可以写自己的工作区，写不到别处。

**选择更宽的授权，意味着约束换了位置。** 不再是"Agent 无法动手"，而是"它动手也够不到
要紧的东西"——依靠的是每用户独立进程、独立 DSH_HOME 与工作区、以及 Runtime 主机本身的
隔离。走这条路的部署需要满足几个前提：

- Runtime 主机是**可牺牲的**：上面没有数据库凭据、没有其他系统的私钥、没有生产数据；
- Gateway 自己的密钥不进入 Runtime 环境。子进程只继承 `PATH`、区域设置、CA 与代理这
  几项运行必需的变量，其余一概留在 Gateway；部署方确实要传给 Runtime 的东西走
  `extraEnv`，让每一个这样的秘密都是一次明确的决定；
- 对公网**没有入站**，只接受来自可信前端的流量；
- 容器层面做过加固（只读根文件系统之外的必要可写卷、`cap_drop`、`no-new-privileges`）。

**同一台主机上的 Subject 之间不是强隔离。** 每个 Runtime 的目录是 `0700`，挡住的是
其他账号与无关进程；但所有 Runtime 共用一个 uid，所以一个 Runtime 若**蓄意**去读兄弟
目录，内核看来它就是属主。要在 Subject 之间做真正的隔离，需要每 Runtime 一个 uid 或
每 Runtime 一个容器 —— 现有实现关掉的是意外暴露，不是蓄意读取。把用户凭据写进工作区
的部署尤其要清楚这一点。

管理面的锁**不随姿态放宽**：禁装插件、禁切 Preset、禁改凭据、禁访问宿主目录，在两种
姿态下都由 Gateway 的 RPC 拦截与 Profile 覆盖层同时约束。放开沙箱不等于放开这些。
