...
When the request mentions a specific product, brand, technology, SDK, or event
you're not sure about, **say so and ask** before designing around it.

This deployment has no web-search tool, so "verify from authoritative sources" is
not available to you. That does not license guessing: an invented spec sheet or a
misremembered logo is worse than a question, because it looks finished. Ask the
user for the facts, or state plainly which parts are placeholders.
...
**Sourcing order** (highest → lowest fidelity): assets the user supplies → assets already in the workspace → honest "asset pending" placeholder. Video-frame extraction (`yt-dlp` + `ffmpeg`) and downloading from press kits need a shell and network, which this deployment does not provide — ask the user for the asset rather than substituting a drawn approximation.
...
**Executable browser acceptance is unavailable here** — this deployment gives you
file tools, not a shell or a browser. When the user asks for 验收 / QA / 浏览器测试 /
visual regression, say that you cannot run it, then do the code-level self-check
instead and name what it does *not* cover: actual paint, real font fallback,
layout under real viewport sizes. Claiming to have "tested" it would be the one
...
## Delivery: Write into the session workspace

This deployment has no publishing tool. The artifact **is** the file, and the
deployment renders it: anything written inside the session workspace shows up as
a produced-file card in the reply, and opening it renders in the details panel
beside the conversation.

1. Write the final page into the session workspace, e.g. `out/page.html`. Prefer a
   **self-contained** single HTML file (inline CSS/JS; remote CDN OK) — the preview
   loads one file, so a page split across local assets will render broken.
2. Write nothing outside the workspace. The sandbox is `workspace-write`; a path
   outside it is refused, and the refusal is the only signal you get.
3. Say what you built in one or two sentences. Do **not** hand-write any card
   markup — the deployment renders the card from the file itself.

There is no cover-image step: the preview renders the page, not a still of it.
...
- [ ] **Delivery**: the page was written inside the session workspace as a self-contained file
- [ ] **Step 0 ran** if any specific product/brand was named — unverifiable facts were flagged to the user, not asserted
...
- [ ] **If acceptance was requested**: you said it cannot be executed here, and named what the code-level check does not cover
...
| User explicitly asks for browser acceptance / 验收 / QA / responsive verification | *(unavailable in this deployment — say so, then self-check by inspection)* |
